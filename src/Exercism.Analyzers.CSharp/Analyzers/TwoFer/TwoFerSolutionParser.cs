...
namespace Exercism.Analyzers.CSharp.Analyzers.TwoFer
{
    internal static class TwoFerSolutionParser
    {
        public static TwoFerSolution Parse(ParsedSolution solution)
        {
            ...
            var returnedExpression = speakMethod?.ReturnedExpression();
            var speakMethodVariable = speakMethod?.AssignedVariable();
            var twoFerError = TwoFerError(twoFerClass, speakMethod, speakMethodParameter);
            return new TwoFerSolution(solution, twoFerClass, speakMethod, speakMethodParameter, returnedExpression, speakMethodVariable, twoFerError);
        }

        private static TwoFerError TwoFerError(ClassDeclarationSyntax twoFerClass, MethodDeclarationSyntax speakMethod, ParameterSyntax speakMethodParameter)
        {
            if (twoFerClass.UsesOverloads())
                return TwoFer.TwoFerError.UsesOverloads;

            if (speakMethod.MissingSpeakMethod())
                return TwoFer.TwoFerError.MissingSpeakMethod;
                
            if (speakMethod.InvalidSpeakMethod(speakMethodParameter))
                return TwoFer.TwoFerError.InvalidSpeakMethod;

            if (speakMethod.UsesDuplicateString())
                return TwoFer.TwoFerError.UsesDuplicateString;

            if (speakMethod.NoDefaultValue())
                return TwoFer.TwoFerError.NoDefaultValue;

            if (speakMethodParameter.UsesInvalidDefaultValue())
                return TwoFer.TwoFerError.InvalidDefaultValue;

            if (speakMethod.UsesStringReplace())
                return TwoFer.TwoFerError.UsesStringReplace;

            if (speakMethod.UsesStringJoin())
                return TwoFer.TwoFerError.UsesStringJoin;

            if (speakMethod.UsesStringConcat())
                return TwoFer.TwoFerError.UsesStringConcat;

            return TwoFer.TwoFerError.None;
        }
